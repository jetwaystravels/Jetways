    ...
    public class AkasaAirCommitBookingController : Controller
    {
		...
		public async Task<IActionResult> AkasaAirBookingView()
        {
			...
			using (HttpClient client = new HttpClient())
			{
				...
				//	var _responceGetBooking = AKresponceGetBooking.Content.ReadAsStringAsync().Result;
				...
				//client.DefaultRequestHeaders.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
				//client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
				//HttpResponseMessage responcepnrPNRBooking = await client.GetAsync(AppUrlConstant.AkasaAirPNRBooking + AirLinePNR);
				//if (responcepnrPNRBooking.IsSuccessStatusCode)
				//{
